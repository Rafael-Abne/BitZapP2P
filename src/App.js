...
//importing actions
import { setUser, setUserApi } from "./actions/userAction";
...
function App() {
  const [{ user, userApi }, dispatch] = useStateValue();
  ...
  useEffect(() => {
    const userMultiWhatsapp = () => {
      if(localStorage.getItem('user_multiwhatsapp')){
         let user = localStorage.getItem('user_multiwhatsapp')
         dispatch(setUserApi(JSON.parse(user)))
      }
    }
    userMultiWhatsapp();
  }, []);
            ...
            <div className="app__body">
              <HashRouter>
                <Switch>
                  ...
                  <Route exact path="/contact/:contact/messages">
                    <Hidden only={["xs"]}>
                      {" "}
                      {/* Sidebar component will be hidden in mobile view */}
                      <Sidebar
